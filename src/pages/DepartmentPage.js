...
const DepartmentPage = () => {
  ...
  const handleAdd = async () => {
    ...
    try {
      const response = await axios.post('http://127.0.0.1:8000/api/create', {
        department_name: formData.fullName,
        department_shortname: formData.shortName,
  ...
  const handleUpdate = async () => {
    ...
    try {
      const response = await axios.put(`http://127.0.0.1:8000/api/update/${formData.id}`, {
        department_name: formData.fullName,
        department_shortname: formData.shortName,
        status: formData.status,
      });
  ...
  const handleEdit = (department) => {
    ...
    if (department) {
      setFormData({
        id: department.department_id, // Correct the field here
        fullName: department.department_name,
        shortName: department.department_shortname,
  ...
  const handleDelete = async (department_id) => {
    try {
      await axios.delete(`http://127.0.0.1:8000/api/delete/${department_id}`);
      fetchDepartments();
    } catch (error) {
  ...
  return (
    <div>
      ...
      <table style={styles.table}>
        <thead>
          <tr>
            <th style={styles.tableHeader}>Department Full Name</th>
            <th style={styles.tableHeader}>Department Short Name</th>
        ...
        <tbody>
          {departments.map((department) => (
            <tr key={department.department_id}>
              <td style={styles.tableCell}>{department.department_name}</td>
              <td style={styles.tableCell}>{department.department_shortname}</td>
              <td style={styles.tableCell}>{department.status}</td>
              <td style={styles.tableCell}>
                <button style={styles.editButton} onClick={() => handleEdit(department)}>
                  Edit
                </button>
                <button style={styles.deleteButton} onClick={() => handleDelete(department.department_id)}>
                  Delete
                </button>
              </td>
            </tr>
      ...
      {showPopup && (
        <div style={styles.popupOverlay}>
          <div style={styles.popup}>
            ...
            <h2 style={styles.heading}>{editMode ? 'Edit' : 'Create'} Department</h2>
            <form style={styles.form} onSubmit={handleSave}>
              <div style={styles.formGroup}>
                <label style={styles.label}>Department Full Name</label>
                <input
                  type="text"
                  name="fullName"
              ...
              <div style={styles.formGroup}>
                <label style={styles.label}>Department Short Name</label>
                <input
                  type="text"
                  name="shortName"
              ...
              <div style={styles.formGroup}>
                <label style={styles.label}>Department Status</label>
                <div>
