...
const AssignToModulePage = () => {
  ...
  const handleAdd = async () => {
    try {
      const response = await axios.post("http://127.0.0.1:8000/api/assign", {
        lecture_name: formData.lecture_id,
        department_name: formData.department_id,
        module_name: formData.module_id,
      });
  ...
  const handleUpdate = async () => {
    if (!formData.id) {
      // Correctly check for formData.id
      alert("ID is missing!");
      return;
    }
    ...
    try {
      const response = await axios.put(
        `http://127.0.0.1:8000/api/assign/${formData.id}`,
        {
          lecture_name: formData.lecture_id,
          department_name: formData.department_id,
          module_name: formData.module_id,
  ...
  const handleDelete = async (assigntomodule_id) => {
    try {
      console.log("Deleting Assign to Module with ID:", assigntomodules.id); // Debug log
      if (!assigntomodule_id) {
        console.error("Invalid assigntomodule_id provided.",assigntomodule_id);
        return;
      }
      await axios.delete(
        `http://127.0.0.1:8000/api/assign/${assigntomodule_id}`
      );
            ...
            <tr key={assigntomodule.id}>
              ...
              <td style={styles.tableCell}>
                <button
                  ...
                  onClick={() => handleEdit(assigntomodule)}
                >
                  Edit
                </button>
                <button
                  style={styles.deleteButton}
                  onClick={() => handleDelete(assigntomodule.id)}
                >
                  Delete
                </button>
              </td>
        ...
        <div style={styles.popupOverlay}>
          <div style={styles.popup}>
            ...
            <h2 style={styles.heading}>
              {editMode ? "Edit" : "Create"} Assignment
            </h2>
            <form style={styles.form} onSubmit={handleSave}>
              <div style={styles.formGroup}>
                <label style={styles.label}>Lecture</label>
                <select
                  name="lecture_id"
